...
void lp_print_usage(int source) {
    ...
    printf("LGProxy v%d.%d.%d build %s %s\n"
           "Built with TCM %lu.%lu.%lu + LG %s (kvmfr v%d)\n"
           ...
           LP_BUILD_VERSION, source ? "(source)" : "(sink)", TCM_MAJOR(ver),
           TCM_MINOR(ver), TCM_PATCH(ver), LG_BUILD_VERSION, KVMFR_VERSION);
            ...
            if (desc[j].id == opts[i].val) {
                ...
                switch (desc[i].required) {
                    ...
                    case REQUIRED_WITH_VALUE:
                        strcpy(req_arg,
                               "(" HIGHLIGHT("required") ", value required)");
                        break;
                    case OPTIONAL:
                        strcpy(req_arg, "(optional flag)");
                        break;
                    default:
                        throw EIO;
                }
...
uint64_t parse_mem_string(char * data) {
    ...
    int      val;
    char     unit[2];
    int      ret = sscanf("%d%1s", data, &val, &unit);
    if (ret < 0 || val <= 0) {
...
int64_t parse_interval_string(char * data) {
    ...
    switch (unit[0]) {
        ...
        case 's': /* Seconds*/
            return val * 1000000;
        default:
            lp_log_error("Unknown delay unit %c", unit[0]);
            throw EINVAL;
    }
}
...
uint32_t parse_version_string(char * data) {
    ...
    int maj, min;
    int ret = sscanf("%d.%d", data, &maj, &min);
...
int check_valid_opts(char * reqd, char * pass, struct option * opts,
    ...
    for (int i = 0; i < n_opts; i++) {
        if (desc[i].required == REQUIRED_WITH_VALUE) {
            if (reqd[i] && !pass[i]) {
                lp_log_warn("Required argument -%c / --%s is missing",
...
lp_sink_opts::lp_sink_opts(int argc, char ** argv) {
    ...
    if (!shortopts) {
        throw tcm_exception(ENOMEM, __FILE__, __LINE__,
                            "Failed to generate option string");
    }
    ...
    while ((o = getopt_long(argc, argv, shortopts, opts, &idx)) != -1) {
        ...
        switch (o) {
            ...
            case 'v':
                ...
                if (!this->fabric_version) {
                    ...
                    throw tcm_exception(
                        ...
                        "Libfabric version string parse failed");
                }
                break;
            case 'b':
            ...
            case 'm':
                ...
                break;
            case 'l':
                this->shm_len = atoi(optarg);
                if (!this->shm_len)
                    throw tcm_exception(EINVAL, __FILE__, __LINE__,
                                        "Shared memory file length invalid");
                break;
            case 'i':
                this->interval = parse_interval_string(optarg);
                break;
            case 't':
                this->timeout = parse_interval_string(optarg) / 1000;
                break;
            case 'h':
                lp_log_debug("Help message requested, stopping");
                errno = EAGAIN;
                throw tcm_exception(EAGAIN, "Help was requested");
            default:
                lp_log_warn("Invalid argument detected");
                break;
        }
    }
    ...
    if (!check_valid_opts(arg_reqd, arg_pass, opts, desc, n_opts))
        throw tcm_exception(EINVAL, __FILE__, __LINE__, "Missing options");
    return;
}
